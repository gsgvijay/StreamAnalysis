...
public class RealTimeElasticSearchTopology {
    ...
    public static StormTopology buildTopology(String[] args, Settings settings, LocalDRPC drpc)
		...
		**/
		...
		// Passed as args from the main method
    ...
    public static void main(String[] args) throws Exception {
	   ...
	   ***/
	   ...
	   //The Strings query1, query2 and query3 are built as Term Queries
       String query1 = QueryBuilders.termQuery("text", "man").buildAsBytes().toUtf8();
	   String query2 = QueryBuilders.termQuery("text", "countdown").buildAsBytes().toUtf8();
	   String query3 = QueryBuilders.termQuery("text", "news").buildAsBytes().toUtf8();
	   
	   //The String query4 is built as a match query which matches in all the given fields
	   String query4 = QueryBuilders.matchQuery("_all", "cwc15").buildAsBytes().toUtf8();

	   //The String query5 is built as a wildcard query which searches the text of a tweet for the given regex
	   String query5 = QueryBuilders.wildcardQuery("text", "New*").buildAsBytes().toUtf8();

	   //The String query6 is built as a prefix query which searches the prefix of a given tweet for the substring
	   String query6 = QueryBuilders.prefixQuery("text", "#cwc15").buildAsBytes().toUtf8();

	   
       String drpcResult;
       for (int i = 0; i < 5; i++) {
	      //Perform the search on operation on the running elasticsearch process
